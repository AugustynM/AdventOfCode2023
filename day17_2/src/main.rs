...
fn dijkstra(map: &Vec<Vec<u32>>) -> Option<u32> {
    ...
    let mut distance: Vec<Vec<Vec<Vec<u32>>>> =
        vec![vec![vec![vec![u32::MAX; 11]; 4]; map[0].len()]; map.len()];
    ...
    let out = distance.last()?.last()?.iter().fold(u32::MAX, |min, a| {
        *a.iter().skip(4).min().unwrap().min(&min)
    });
    if out < u32::MAX {
        Some(out)
    } else {
